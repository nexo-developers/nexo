...
CvMemStorage* circleStorage = cvCreateMemStorage(0);
 // Creamos interface de Usuario ..........
   ...
   // Dilatacion y Erocion
    ...
    IplImage* imageOriginal = cvLoadImage(argv[1],CV_LOAD_IMAGE_COLOR);
    if(imageOriginal==NULL){
    	fprintf(stderr, "imagen null!\n");
    	exit(1);
    }
    frame = cvCreateImage(cvGetSize(imageOriginal),imageOriginal->depth,imageOriginal->nChannels);
   while(1) {

      cvCopy(imageOriginal,frame,NULL); // Obtenemos Frame desde el Buffer de Video
