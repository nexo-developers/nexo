...
char                *config_name = "Data/MostrarAnimales/prueba.patterns";
...
int anchoPantalla = 300;
int altoPantalla = 240;
    ...
    if(!pui->capturarImagenAR()){
		...
		if(err_deteccion==DETECCION_CORRECTA){
			...
			if(idObjetoDetectado!=-1){
				...
				//actualizarPantalla();
				if(idObjetoPUIActual == 0){
					createRelativeMouseEvent(0,-10);
				} else if(idObjetoPUIActual == 1){
					createRelativeMouseEvent(0,10);
				}else if(idObjetoPUIActual == 2){
					createRelativeMouseEvent(-10,0);
				}else if(idObjetoPUIActual == 3){
					//createRelativeMouseEvent(10,0);
					mouseClick( Button1);
				}
			}
...
//	cantElementosCargados = pui->leerConfiguracionObjetosPUI("Data/MostrarAnimales/mostrarAnimales_ModoSimple.objetosPUI",MAX_MARCADORES);
	cantElementosCargados = pui->leerConfiguracionObjetosPUI("Data/MostrarAnimales/prueba.objetosPUI",MAX_MARCADORES);
	...
	CargarFondo("Data/MostrarAnimales/fondoPrototipo1.bmp",1);
