...
/** @new Gonza, Marce, Seba
 * \brief pause the actual video source.
 *
 * Pauses the current video source.
 * \pre arVideoOpen previously invoked
...
/*smarichal
 * \brief get the interval of time in wich the new data should be updated depending of the framerate stablished
 *in the gstreamer pipeline
 *
 * \param frateInterval a pointer to the frame rate interval
 * \return 0 if the framerate is setted in pipeline, otherwise -1
 * */
AR_DLL_API  int				arVideoInqFramerateInterval(double *frateInterval);
